# AGENTS.md — Autonomous App Development System
...
> **Using Claude Code?** `CLAUDE.md` contains identical orchestrator instructions. Both tools are fully supported.
...
> Every implementation task MUST be delegated to the appropriate specialist sub-agent.
> Doing the work yourself instead of delegating is a failure of the orchestrator role.

Spawn each specialist agent with:
- The agent's system prompt (read from `agents/<name>.md`) + relevant state files
- Current state context (active story, task, acceptance criteria)
...
AGENTS.md              # This file (also mirrored in CLAUDE.md)
